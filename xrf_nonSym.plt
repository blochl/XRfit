...
f1(x) = ( (x<=b1) ? lr(na1)*fl(x,a1,b1,cla1) + (1-lr(na1))*fg(x,a1,b1,cga1) : lr(nb1)*fl(x,a1,b1,clb1) + (1-lr(nb1))*fg(x,a1,b1,cgb1) )
...
############ Edit - Initial values ############
### First peak
cla1 = 0.032; cga1 = 0.032; # Broadening
clb1 = 0.022; cgb1 = 0.022;
a1 = 0.9;                 # Height
b1 = 10.500;              # Center 
...
fit [ XMIN : XMAX ] f(x) FILE u 1:($2/STATS_max):($3/STATS_max) via\
    cla1, cga1, clb1, cgb1, a1, b1, na1, nb1, m, l\
#    , cl2, cg2, a2, b2, n2\
#    , cl3, cg3, a3, b3, n3
###############################################
###############################################
...
set label sprintf("{/Symbol \\150} = %.3f", 0.5*(lr(na1)+lr(nb1))) at b1 , graph 0.22
set label sprintf("L = %.3f {/Symbol \\261} %.3f nm", 0.5*(Size(b1,cla1)+Size(b1,clb1)), 0.5*(SizeError(b1,b1_err,cla1,cla1_err) + SizeError(b1,b1_err,clb1,clb1_err))) at b1 , graph 0.17
set label sprintf("{/Symbol \\163} = %.3e {/Symbol \\261} %.3e", 0.5*(Microstrain(b1,cga1)+Microstrain(b1,cgb1)), 0.5*(MicrostrainError(b1,b1_err,cga1,cga1_err) + MicrostrainError(b1,b1_err,cgb1,cgb1_err))) at b1 , graph 0.12
###############################################
### Touch this when number of peaks changes ###
## Second peak (COMMENT IF NOT NEEDED):
...
print "nu1 = ",lr(na1),"; +",lr(na1+na1_err)-lr(na1),lr(na1-na1_err)-lr(na1)
print "FWHM G1 = ",2*sqrt(2*log(2))*cga1
print "FWHM L1 = ",2*cla1
print "Size1 = ",Size(b1,cla1)," nm"
###############################################
### Touch this when number of peaks changes ###
## Second peak (COMMENT IF NOT NEEDED):
